...
import {
  ...
  Award, Loader2, Wrench,
  Shield, Briefcase, Calendar, AlertCircle, Heart
} from 'lucide-react'
...
function AvailabilityBadge({ isAvailable, size = 'sm' }) {
  if (isAvailable) {
    return (
      <span className={`flex items-center gap-1 font-semibold text-green-600 ${size === 'sm' ? 'text-[10px]' : 'text-xs'}`}>
  ...
  return (
    <span className={`flex items-center gap-1 font-semibold text-gray-400 ${size === 'sm' ? 'text-[10px]' : 'text-xs'}`}>
...
// ─── reserve button ───────────────────────────────────────────────────────────
// Shows tooltip if handyman is unavailable; still allows booking

function ReserveButton({ service, handyman, onBook, size = 'sm' }) {
  const [showTip, setShowTip] = useState(false)

  const handleClick = (e) => {
    e.preventDefault()
    e.stopPropagation()
    onBook(service, handyman)
  }
  ...
  if (handyman.is_available) {
    return (
      <button
        onClick={handleClick}
        className={`flex items-center justify-center gap-1.5 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition
  ...
  return (
    <div className="relative">
      ...
      {showTip && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-20 w-52 bg-gray-800 text-white text-xs rounded-xl p-2.5 shadow-xl pointer-events-none">
          <div className="flex items-start gap-1.5">
            <AlertCircle className="w-3.5 h-3.5 text-yellow-400 flex-shrink-0 mt-0.5" />
            <span>Handymanul e indisponibil momentan. Cererea va fi trimisă și el va confirma când devine disponibil.</span>
          </div>
...
const URGENCY_MAP = { low: 'text-gray-400', normal: 'text-gray-500', medium: 'text-yellow-600', high: 'text-red-600' }
...
export default function FindServices() {
  ...
  // booking modal
  const [bookingTarget,    setBookingTarget]    = useState(null) // { service, handyman }
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: myProfile } = await supabase
        .from('profiles').select('city, county').eq('id', user.id).maybeSingle()
      ...
      const { data: cats } = await supabase
        .from('categories').select('id, name, slug, icon').eq('is_active', true).order('name')
      setCategories(cats ?? [])
      ...
      const [{ data: pRows }, { data: svcRows }] = await Promise.all([
        ...
        supabase.from('handyman_services')
          ...
          .in('handyman_id', userIds).eq('is_available', true),
      ])
      ...
      // Încarcă favoriții clientului
      const { data: favData } = await supabase
        .from('favorite_handymen')
        .select('handyman_id')
        .eq('client_id', user.id)
      setFavorites(new Set((favData ?? []).map(f => f.handyman_id)))
  ...
  const filtered = useMemo(() => {
    return handymen.filter(h => {
      if (searchQuery) {
        ...
        if (!h.full_name.toLowerCase().includes(q) &&
            !(h.specialties ?? []).some(s => s.toLowerCase().includes(q)) &&
            !h.services.some(s => s.title.toLowerCase().includes(q))) return false
      }
      if (selectedCategory !== 'all') {
        if (!h.services.some(s => s.category_id === Number(selectedCategory))) return false
      }
  ...
  const activeFilterCount = [
    filters.availability !== 'all', filters.minRating > 0, filters.maxPrice !== '',
    filters.verifiedOnly, filters.insuredOnly, selectedCategory !== 'all',
  ].filter(Boolean).length

  // ── open booking modal ─────────────────────────────────────────────────────
  const handleBook = (service, handyman) => {
    setBookingTarget({ service, handyman })
  }

  // ── toggle favorite ────────────────────────────────────────────────────────
  const toggleFavorite = async (e, handymanId) => {
    e.preventDefault()
    e.stopPropagation()
    if (!userId) return
    const isFav = favorites.has(handymanId)
    if (isFav) {
      await supabase.from('favorite_handymen')
        .delete()
        .eq('client_id', userId)
        .eq('handyman_id', handymanId)
      setFavorites(prev => { const s = new Set(prev); s.delete(handymanId); return s })
    } else {
      await supabase.from('favorite_handymen')
        .insert({ client_id: userId, handyman_id: handymanId })
      setFavorites(prev => new Set(prev).add(handymanId))
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <button onClick={() => setShowFilters(o => !o)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition relative ${showFilters ? 'bg-blue-600 text-white' : 'border border-gray-300 text-gray-700 hover:bg-gray-50 bg-white'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {featured.map(h => (
                <Link key={h.user_id} to={`/handymen/${h.slug}`}
                  ...
                  <span className="absolute top-4 right-4 px-2 py-0.5 bg-blue-100 text-blue-600 text-xs font-bold rounded-full">⭐ Recomandat</span>
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500 pt-3 border-t border-gray-100">
                    <span className="flex items-center gap-1"><Wrench className="w-3 h-3" />{h.total_jobs_completed ?? 0} lucrări</span>
                    <AvailabilityBadge isAvailable={h.is_available} />
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-gray-500">
              <span className="font-semibold text-gray-800">{filtered.flatMap(h => h.services).length}</span> servicii găsite de la{' '}
              <span className="font-semibold text-gray-800">{filtered.length}</span> handymani
                ...
                {allServices.map(svc => {
                  ...
                  return (
                    <div key={svc.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all overflow-hidden group">
                      {/* availability banner for unavailable */}
                      {!h.is_available && (
                        <div className="bg-gray-100 border-b border-gray-200 px-4 py-1.5 flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-gray-400" />
                          <span className="text-xs text-gray-500 font-medium">Handyman indisponibil momentan</span>
                        </div>
                      )}
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500 pb-3 mb-3 border-b border-gray-100">
                          ...
                          {svc.estimated_duration && (
                            <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-gray-400" />{svc.estimated_duration}</span>
                          )}
                          {svc.is_popular && (
                            <span className="ml-auto px-2 py-0.5 bg-yellow-50 text-yellow-600 font-semibold rounded-full border border-yellow-100 text-[10px]">🔥 Popular</span>
                          )}
                        </div>
                        ...
                        <div className="flex gap-2">
                          <ReserveButton service={svc} handyman={h} onBook={handleBook} size="full" />
                          <button
                            onClick={e => toggleFavorite(e, h.user_id)}
                            className={`w-9 h-9 flex items-center justify-center border rounded-xl transition flex-shrink-0
                              ${favorites.has(h.user_id)
                                ? 'border-red-300 bg-red-50 text-red-500 hover:bg-red-100'
                                : 'border-gray-200 text-gray-400 hover:text-red-500 hover:border-red-300'}`}
                          >
                            <Heart className={`w-4 h-4 ${favorites.has(h.user_id) ? 'fill-red-500' : ''}`} />
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  return (
                    <div key={svc.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all overflow-hidden flex">
                      <div className="w-36 flex-shrink-0 relative">
                        {!h.is_available && (
                          <div className="absolute inset-0 bg-gray-400/20 z-10 flex items-end pb-2 justify-center">
                            <span className="text-[10px] bg-gray-800/70 text-white px-2 py-0.5 rounded-full">Indisponibil</span>
                          </div>
                        )}
                        {Array.isArray(svc.photos) && svc.photos[0]
                      ...
                      <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2 ml-2 flex-shrink-0">
                            <ReserveButton service={svc} handyman={h} onBook={handleBook} size="sm" />
                            <button
                              onClick={e => toggleFavorite(e, h.user_id)}
                              className={`w-9 h-9 flex items-center justify-center border rounded-xl transition flex-shrink-0
                                ${favorites.has(h.user_id)
      ...
      {bookingTarget && (
        <BookingModal
          ...
          handyman={bookingTarget.handyman}
          userId={userId}
